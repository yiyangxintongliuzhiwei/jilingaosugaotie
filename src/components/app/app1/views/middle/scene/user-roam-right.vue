<template>
  ...
  <view-template id="user-roam-chart-wrapper">
    ...
    <div id="user-roam-chart-right" class="user-roam-chart-right"></div>
    <div class="numList">
...
export default {
  ...
  methods: {
    initCharts() {
      let chart = echarts.getInstanceByDom(
        document.getElementById('user-roam-chart-right')
      );
    ...
    changeArea(name) {
      this.$http.indi.get('j0047', {'region_name': name}, (res) => {
        this.cityNameArr = []
        this.cityData = []
  ...
  mounted() {
    echarts.init(document.getElementById('user-roam-chart-right'));
